...
#define GVTV_SH_TRANS
#ifdef GVTV_SH_TRANS	
#include <sys/syscall.h>

#define __NR_fallocate		(__NR_SYSCALL_BASE+352)

int gvtv_sh_fallocate64( int fd, int mode, uint64_t offset, uint64_t len )
{
	return syscall( __NR_fallocate, fd, mode, (int)offset, 0, (int)len, 0);
}
#endif
...
preallocate_file_sparse( int fd, uint64_t length )
{
    ...
    if( !success ) /* fallocate64 is always preferred, so try it first */
		...
		if( !success ) /* fallocate64 is always preferred, so try it first */
{
			success = !gvtv_sh_fallocate64( fd, 0, 0, length );
printf("############# gvtv_sh_fallocate64 fd=%d, length=%lld Mb\n", fd, length/1024/1024);
}
#endif
